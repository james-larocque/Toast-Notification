...
try {
    ...
    $ActionToastEnabled = $Xml.Configuration.Feature | Where-Object { $_.Name -like 'Toast' } | Select-Object -ExpandProperty 'HasAction'
    if ($ActionToastEnabled -eq "True") {
        $ActionToast = $Xml.Configuration.Feature | Where-Object { $_.Name -like 'Toast' } | Select-Object -ExpandProperty 'Action'
    }
...
$OpenToastXMLTag = "<toast scenario=`"$Scenario`">"

if ($ActionToastEnabled -eq "True") {
    $OpenToastXMLTag = "<toast scenario=`"$Scenario`" activationType=`"protocol`" launch=`"$ActionToast`">"
}
